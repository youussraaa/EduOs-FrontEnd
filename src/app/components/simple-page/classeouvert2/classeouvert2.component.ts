import { Component, HostListener, NO_ERRORS_SCHEMA, OnInit, TemplateRef, ViewChild } from "@angular/core";
...
@Component({
  selector: "app-classeouvert2",
  templateUrl: "./classeouvert2.component.html",
  styleUrl: "./classeouvert2.component.scss",
})
export class Classeouvert2Component implements OnInit {
  ...
  onWindowResize($event) {
    ...
    setTimeout(() => {
      ...
      this.gridConfig.height = window.innerHeight - 210 + "px";
  ...
  gridConfig: any = {
    ...
    columns: [
      ...
      {
        field: "NbEtudiantClasseAnnees",
        header: "Nombre inscrits",
        dataType: "number",
        visible: true,
        sortable: true,
        resizable: true,
  ...
  GetData() {
    ...
    this.EduosService.GetClasseAnnee(params).subscribe(
      (response) => {
        console.log("response GetClasseAnnee de getData ", response);
        this.loader.hide();

        if (response == null) {
          this.toastr.error("Erreur de GetClasseAnnee");
        } else if (response.length == 0) {
          this.toastr.info("Aucune classe ouverte trouvée");
        }
  ...
  ngOnInit(): void {
    ...
    this.EduosService.GetCommunData([
      ...
      `${environment.apiUrl}/api/Configuration/GetEnseignants?Ecole_Id=${this.EcoleId}`,
    ]).subscribe(
      (response: any[]) => {
        console.log("Response GetCommunData: ", response);
        this.Enums.Annee = response[0];
        this.Enums.Enseignant = response[1];
      },
      (error) => {
  ...
  dialogClasseAnneeMatieres: {
    ...
    Matieres: [],
    Enseignants: [],
    Open: (ClasseAnn_Id: string, modal) => {
      // console.log("ctx.data : ", ctx.data);
      this.dialogClasseAnneeMatieres.ClasseAnnee = this.gridConfig.data.find((x) => x.ClasseAnn_Id == ClasseAnn_Id);
      // this.newClasseAnneeMatiere.ClasseAnn_Id = ctx.data.ClasseAnn_Id;

      this.loader.show();
      this.EduosService.GetClasseAnneeMatiere(ClasseAnn_Id).subscribe(
        (response) => {
          this.loader.hide();
        ...
        (error) => {
          console.error("Erreur GetClasseAnneeMatiere: ", error);
          this.loader.hide();
      ...
      this.EduosService.GetMatieres(this.dialogClasseAnneeMatieres.ClasseAnnee.Niv_Id).subscribe(
        (response) => {
          console.log("response GetMatieres: ", response);
          if (response == null) {
            this.toastr.error("Erreur de GetMatieres");
          } else {
            this.dialogClasseAnneeMatieres.Matieres = response;
          }
        },
        (error) => {
      ...
      this.EduosService.GetEnseignants().subscribe(
          (response) => {
            ...
            if (response == null) {
              ...
              this.dialogClasseAnneeMatieres.Enseignants = response;
              console.log("this.dialogClasseAnneeMatieres.Enseignants : ", this.dialogClasseAnneeMatieres.Enseignants);
              this.loader.hide();
            }
          },
          (error) => {
    ...
    Clear: () => {
      ...
      this.dialogClasseAnneeMatieres.Matieres = [];
      this.dialogClasseAnneeMatieres.Enseignants = [];
    },
    Close: (modal: any) => {
    ...
    SubmitAddMatiere: (modal: any) => {
      ...
      this.dialogClasseAnneeMatieres.newClasseAnneeMatiere.Mat_Nom = this.dialogClasseAnneeMatieres.Matieres.find((x) => x.Mat_Id == this.dialogClasseAnneeMatieres.newClasseAnneeMatiere.Mat_Id)?.Mat_Nom;
      this.dialogClasseAnneeMatieres.newClasseAnneeMatiere.Ens_Nom = this.dialogClasseAnneeMatieres.Enseignants.find((x) => x.Ens_Id == this.dialogClasseAnneeMatieres.newClasseAnneeMatiere.Ens_Id)?.Ens_Nom;
      this.dialogClasseAnneeMatieres.newClasseAnneeMatiere.Ens_Prenom = this.dialogClasseAnneeMatieres.Enseignants.find((x) => x.Ens_Id == this.dialogClasseAnneeMatieres.newClasseAnneeMatiere.Mat_Id)?.Ens_Prenom;
          ...
          if (response == null || response == false) {
            ...
            this.dialogClasseAnneeMatieres.ClasseAnneeMatieres = this.dialogClasseAnneeMatieres.ClasseAnneeMatieres.map((item) => {
              if (this.dialogClasseAnneeMatieres.ClasseAnneeMatiereToEdit.ClsAnnMat_Id == item.ClsAnnMat_Id) {
                item.Ens_Nom = this.dialogClasseAnneeMatieres.Enseignants.find((x) => x.Ens_Id == this.dialogClasseAnneeMatieres.ClasseAnneeMatiereToEdit.Ens_Id)?.Ens_Nom;
                item.Ens_Prenom = this.dialogClasseAnneeMatieres.Enseignants.find((x) => x.Ens_Id == this.dialogClasseAnneeMatieres.ClasseAnneeMatiereToEdit.Ens_Id)?.Ens_Prenom;
              }
              return item;
            });
            modal.close();
  ...
  deleteClasseAnnee(ClasseAnn_Id: any) {
    const id = ClasseAnn_Id;
    this.loader.show();
  ...
  dialogClasseAnnee: {
    ...
    toggleAllSelections: () => {
      this.dialogClasseAnnee.ClasseAOuvrir.forEach((classe) => (classe.selected = this.dialogClasseAnnee.isSelectAllChecked));
    },
    Open: (modal) => {
    ...
    Clear: () => {
      ...
      this.dialogClasseAnnee.isSelectAllChecked = false;
    },
    Close: (modal: any) => {
      ...
      this.EduosService.CreateClasseAnnee(this.dialogClasseAnnee.selectedClasseOuvrir).subscribe(
        (response) => {
          if (response == null || response == false) {
            ...
            //push only
            this.dialogClasseAnnee.ClasseAOuvrir.forEach((classe) => {
              if (classe.selected) {
                console.log("this.dialogClasseAnneeMatieres :",this.dialogClasseAnneeMatieres.ClasseAnnee);
                this.gridConfig.data.push({
                  ClasseAnn_Id: classe.ClasseAnn_Id,
                  Ann_Id: classe.Ann_Id,
                  Cls_Id: classe.Cls_Id,
                  Niv_Nom: classe.Niv_Nom,
                  Ann_Nom: this.gridConfig.data.find((x) => x.Ann_Id == classe.Ann_Id)?.Ann_Nom,
                  Cls_Nom: classe.Cls_Nom,
                  Classe_Description: classe.Classe_Description,
                  NbClasseAnneeMatieres: classe.NbClasseAnneeMatieres,
                  NbEtudiantClasseAnnees: classe.NbEtudiantClasseAnnees
                });
              }
            });
            // this.EduosService.GetClasseAnnee(params).subscribe(
    ...
    deleteClasseAnnee: (ClasseAnn_Id: any) => {
      ClasseAnn_Id = ClasseAnn_Id;
      this.loader.show();
      // Vérification préalable via CanDelete
      this.EduosService.CanDelete(ClasseAnn_Id, "ClasseAnnee").subscribe(
        (response) => {
          this.loader.hide();
          if (response == null) {
            this.toastr.error("Erreur lors de la vérification de la suppression.", "Erreur");
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                ...
                // Appel du service de suppression
                this.EduosService.DeleteClasseAnnee([ClasseAnn_Id]).subscribe(
                  (response) => {
                    this.loader.hide();
                    if (response == null || response == false) {
                      this.toastr.error("Erreur de suppression de la classe ouverte");
                      ...
                      // Suppression de la classe de la liste affichée
                      this.gridConfig.data = this.gridConfig.data.filter((item) => item.ClasseAnn_Id !== ClasseAnn_Id);
                      this.toastr.success("Classe ouverte supprimée avec succès");
                    }
