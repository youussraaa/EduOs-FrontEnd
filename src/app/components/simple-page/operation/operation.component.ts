...
export class OperationComponent implements OnInit {
  ...
  openReglementModal(ReglementModal, args) {
    // console.log("===> ", args);
    let data = this.gridConfig.data.find((x) => x.Ope_Id == args.key);
  ...
  @HostListener("window:resize", ["$event"])
    onWindowResize($event) {
      ...
      setTimeout(() => {
        ...
        let cardInfoHeight = document.getElementById("infoCard")?.clientHeight;
        if (searchBarHeight == null || searchBarHeight == undefined || searchBarHeight == 0) {
          searchBarHeight = -25;
        }
        if (cardInfoHeight == null || cardInfoHeight == undefined || cardInfoHeight == 0) {
          console.log(cardInfoHeight)
          cardInfoHeight = -30;
          console.log(cardInfoHeight)
        }
        // searchBarHeight -= 20;
        this.gridConfig.height = window.innerHeight - searchBarHeight - cardInfoHeight - 260 + "px";
      }, 100);
    }
  // exportExcel() {
